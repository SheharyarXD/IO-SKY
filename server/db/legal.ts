 ...
 * server/db/legal.ts
 *
 * Database helpers for the Legal & Consent domain.
 * Covers: legal documents, agreement versions, acceptances,
 * cookie consents, and legal acknowledgements.
 *
 * NOTE: This module consolidates server/legalDb.ts into the unified
 * server/db/ directory. The original server/legalDb.ts is preserved as a
 * re-export shim to avoid breaking existing imports.
 */
...
export async function userHasAcceptedLatest(
  ...
  const db = await getDb();
  if (!db)
    return { document: live.document, version: live.version, accepted: false };
  const rows = await db
    .select({ id: agreementAcceptances.id })
    .from(agreementAcceptances)
    .where(
